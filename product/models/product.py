...
class Product(TimeDeletedStampsWithOrder):
    ...
    inventory = models.OneToOneField(
        ProductInventory, on_delete=models.CASCADE, related_name="inventory_product"
    )
    price = models.DecimalField(decimal_places=2, default=0, max_digits=16)
    discount = models.ForeignKey(
        ProductDiscount, on_delete=models.CASCADE, related_name="discount_products"
    )
    delivery_type = models.CharField(
        max_length=30, choices=DeliveryType.choices, default=DeliveryType.INSTOREPICKUP
    )
    shipping_type = models.CharField(
        max_length=30, choices=ShippingType.choices, default=ShippingType.POSTALSERVICE
    )
