...
class Payment(TimeStampsWithOrder):
    ...
    status = models.CharField(choices=StatusOrder.choices, default=StatusOrder.PROCESSING,max_length=30)
...
class Order(UserOnToOne,TimeStampsWithOrder):
    ...
    payment = models.OneToOneField(Payment, on_delete=models.CASCADE, related_name = "payment_order")    
...
class OrderItem(TimeStampsWithOrder):
    order = models.ForeignKey(Order, related_name="order_item_orders", on_delete=models.CASCADE)
    product= models.OneToOneField(Product, related_name="product_order", on_delete=models.CASCADE)
